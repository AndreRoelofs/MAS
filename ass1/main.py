...
def parse_vector(pref_vec):
    ...
    if pref_vec[:2] != '[[' or pref_vec[-2:] != ']]':
        print("Preference should be a matrix starting with [[ and ending with ]]")
        return None
    ...
    if len(pv.shape) != 2:
        print("Preference has more than two dimensions")
        return None
            ...
            if event.type == pygame.USEREVENT:
                ...
                if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                    ...
                    if event.ui_element == randomize_n_preferences_button:
                        set_n_preferences(random.randint(1, 20))
                        generate_random_preference_matrix()
